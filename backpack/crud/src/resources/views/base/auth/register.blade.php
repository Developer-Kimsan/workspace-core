...
@section('content')
    <div class="row justify-content-center">
        <div class="col-12 col-md-8 col-lg-4">
            <h3 class="text-center mb-1">{{ trans('backpack::base.register') }}</h3>
            <div class="text-right col-sm-12"><a href="{{ backpack_url('dashboard') }}">{{ trans('backpack::base.main_dashboard') }}</a></div>
            <div class="card">
                <div class="card-body">
                    <form class="col-md-12 p-t-10" role="form" method="POST" action="{{ route('backpack.auth.register') }}">
                        ...
                        <div class="form-group">
                            <label class="control-label" for="role">{{ trans('backpack::base.role') }}</label>

                            <div>
                                {{-- <input type="password" class="form-control{{ $errors->has('password_confirmation') ? ' is-invalid' : '' }}" name="password_confirmation" id="password_confirmation"> --}}
                                <select name="role" id="role" class="form-control{{ $errors->has('role') ? ' is-invalid' : '' }}">
                                    <option value="2" selected>{{ trans('backpack::base.user_role') }}</option>
                                    <option value="1">{{ trans('backpack::base.admin_role') }}</option>
                                </select>
                                @if ($errors->has('role'))
                                    <span class="invalid-feedback">
